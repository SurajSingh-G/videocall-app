...
function Dashboard() {
  ...
  const startCall = async () => {
    try {
      ...
      // Ensures that the audio track is enabled
      currentStream.getAudioTracks().forEach(track => { track.enabled = true });
      setIsSidebarOpen(false);
      setCallRejectedPopUp(false);
      //  console.log(`Calling to ${showReciverDetails.username}`);

      const peer = new Peer({
        initiator: true,   // This user starts teh call.
        trickle: false,    //Prevents trickling of ICE candidate , ensuring a single signal exchange.
        stream: currentStream  //Attach  the Local media server
      })
      ...
      // for starting stream another side
  ...
  return (
    <div className='flex min-h-screen bg-gray-100'>
      ...
      {/* Sidebar */}
      <aside
        className={`bg-gradient-to-br from-blue-500 to-teal-300 text-white w-64 h-full p-4 space-y-4 fixed z-20 transition-transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0`}
      >
          ...
          {filteredUsers.map((user) => (
            <li
              ...
              className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer ${selectedUser === user._id
                ? "bg-green-600"
                : "bg-gradient-to-r from-blue-600 to-purple-500 0"
                }`}
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-700 truncate w-32"> {user.email} </span>
              </div>
            </li>
          ))}
        </ul>
      ...
      <div className="flex-1 p-6 md:ml-72 text-white">
        ...
        {selectedUser || reciveingCall || callAccepted ? (
          <div className='relative w-full h-full bg-black flex items-center justify-center'>
            ...
            />
            <div className='absolute bottom-[3px] md:bottom-3 right-3 bg-gray-900 rounded-lg overflow-hidden shadow-lg'>
              < video
                ref={myVideo}
                autoPlay
                playsInline
          ...
          <div>
            <div className="flex items-center gap-5 mb-6 bg-gray-800 p-5 rounded-xl shadow-md">
              ...
              <div>
                <h1 className="text-4xl font-extrabold bg-gradient-to-r from-green-500 to-yellow-500 text-transparent bg-clip-text">
      ...
      {showReciverDetailPopUp && showReciverDetails && (
        <div className="fixed inset-0 bg-transparent bg-opacity-30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-300  rounded-lg shadow-lg max-w-md w-full p-6">
            <div className="flex flex-col items-center">
              ...
              <div className="flex gap-4 mt-5">
                ...
                <button
                  ...
                  className="bg-red-400 text-white px-4 py-1 rounded-lg w-28"
                >
                  Cancel
                </button>
              </div>
